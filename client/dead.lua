...
function DeadMovement()
    ...
    if IsDisabledControlJustPressed(0, 32) then
        Wait(100)
        ClearPedTasks(ped)
        TaskPlayAnimAdvanced(ped, "move_injured_ground", "front_loop", GetEntityCoords(ped), 1.0, 1.0, GetEntityHeading(ped), 1.0, 1.0, 1.0, 35, 1.0, 0, 0)
    elseif IsDisabledControlJustReleased(0, 32) then 
        TaskPlayAnimAdvanced(ped, "move_injured_ground", "front_loop", GetEntityCoords(ped), 1.0, 1.0, GetEntityHeading(ped), 1.0, 1.0, 1.0, 34, 1.0, 0, 0)
    end
end
